...
# Nebenstellen-Liste aus sip.conf (ohne Regex mit '[')
list_extensions() {
  awk '
    BEGIN{inblk=0; ext=""; name="-"; tf=0; hd=0}
    # Abschnittszeile beginnt mit '['
    substr($0,1,1)=="[" {
      # Vorherigen Block ggf. ausgeben
      if (inblk && tf && hd && ext!="") print ext " " name
      inblk=0; tf=0; hd=0; ext=""; name="-"
      # Header parsen: [<digits>]
      if (substr($0,length($0),1)=="]") {
        mid=substr($0,2,length($0)-2)
        # Nur rein numerisch und 2..4 Zeichen
        isnum=(mid ~ /^[0-9]+$/)
        if (isnum && length(mid)>=2 && length(mid)<=4) { ext=mid; inblk=1 }
      }
      next
    }
...
sip_set_mailbox() {
  ...
  $0=="[" EXT "]" { print; inblk=1; have=0; next }
  {
    if (inblk && substr($0,1,1)=="[") { if (!have) print "mailbox="EXT"@default"; inblk=0 }
...
sip_remove_mailbox() {
  ...
  awk -v EXT="$ext" '
  BEGIN{inblk=0}
  $0=="[" EXT "]" { print; inblk=1; next }
  {
    if (inblk && substr($0,1,1)=="[") { inblk=0 }
...
ext_block_has_vmline() {
  ...
  $0=="[internal]" { sect=1; inblk=0; next }
  substr($0,1,1)=="[" && $0!="[internal]" { if (sect) sect=0; inblk=0; next }
  sect && $0 ~ /^[ \t]*exten[ \t]*=>[ \t]*/ {
    # Start der Zeile extrahieren und EXT prüfen
    line=$0
    gsub(/^[ \t]*/,"",line)
    if (index(line,"exten => " EXT ",")==1 || index(line,"exten=> " EXT ",")==1 || index(line,"exten =>" EXT ",")==1) inblk=1
    next
  }
  sect && inblk && $0 ~ /Voicemail\(/ { have=1 }
...
ext_add_vmline_after_dial() {
  ...
  $0=="[internal]" { print; sect=1; inblk=0; added=0; next }
  substr($0,1,1)=="[" && $0!="[internal]" { 
    if (sect && inblk && !added) print " same => n,Voicemail("EXT"@default,u)"
    print; sect=0; inblk=0; added=0; next
  }
  {
    if (sect) {
      if ($0 ~ /^[ \t]*exten[ \t]*=>[ \t]*/) {
        line=$0; gsub(/^[ \t]*/,"",line)
        if (index(line,"exten => " EXT ",")==1 || index(line,"exten=> " EXT ",")==1 || index(line,"exten =>" EXT ",")==1) { print; inblk=1; next }
      }
      if (inblk && $0 ~ /Dial\(/) { print; if (!added) { print " same => n,Voicemail("EXT"@default,u)"; added=1 }; next }
    }
    print
  }
...
ext_remove_vmline() {
  ...
  $0=="[internal]" { print; sect=1; inblk=0; next }
  substr($0,1,1)=="[" && $0!="[internal]" { print; sect=0; inblk=0; next }
  {
    if (sect) {
      if ($0 ~ /^[ \t]*exten[ \t]*=>[ \t]*/) {
        line=$0; gsub(/^[ \t]*/,"",line)
...
internal_use_local_helper_for_ext() {
  ...
  $0=="[internal]" { print; sect=1; inblk=0; replaced=0; next }
  substr($0,1,1)=="[" && $0!="[internal]" { print; sect=0; inblk=0; next }
  {
    if (sect) {
      if ($0 ~ /^[ \t]*exten[ \t]*=>[ \t]*/) {
  ...
  END{ if (sect && inblk && !replaced) print " same => n,Dial(Local/"EXT"@calltoext,30,rtT)" }
  ' "$EXT_CONF" > "${EXT_CONF}.new" && mv "${EXT_CONF}.new" "$EXT_CONF"
}
...
ringgroup_get_dial_line() {
  awk '
  BEGIN{inblk=0}
  $0=="[ringall]" { inblk=1; next }
  substr($0,1,1)=="[" && $0!="[ringall]" { if (inblk) exit }
...
ringgroup_set_members() {
  ...
  $0=="[ringall]" { print; inblk=1; next }
  substr($0,1,1)=="[" && $0!="[ringall]" { if (inblk) inblk=0 }
  {
    if (inblk && /Dial\(/) { print " same => n,Dial("NEWDIAL",30,rtT)"; done=1; next }
    print
  }
  END{ if (inblk && !done) print " same => n,Dial("NEWDIAL",30,rtT)" }
  ' "$EXT_CONF" > "${EXT_CONF}.new" && mv "${EXT_CONF}.new" "$EXT_CONF"
}
